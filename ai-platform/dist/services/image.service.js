...
exports.imageModels = void 0;
exports.generateImage = generateImage;
const config_1 = require("../utils/config");
const logger_1 = require("../utils/logger");
// ── Automatic1111 (local Stable Diffusion) ────────────────────────────────────
async function callA1111(opts) {
    const res = await fetch(`${config_1.config.SD_BASE_URL}/sdapi/v1/txt2img`, {
        method: 'POST',
    ...
    if (!res.ok)
        throw new Error(`A1111 ${res.status}: ${await res.text()}`);
    const data = await res.json();
    return {
        images: data.images.map(b64 => `data:image/png;base64,${b64}`),
        model: config_1.config.SD_MODEL,
...
async function callReplicate(opts) {
    ...
    const res = await fetch('https://api.replicate.com/v1/models/stability-ai/sdxl/predictions', {
        ...
        headers: {
            ...
            'Authorization': `Bearer ${config_1.config.REPLICATE_API_TOKEN}`,
            'Prefer': 'wait=60',
        },
        body: JSON.stringify({
            input: {
...
async function generateImage(opts) {
    try {
        ...
        logger_1.logger.warn({ err }, 'Automatic1111 unavailable — falling back to Replicate');
        return callReplicate(opts);
    }
}
const imageModels = () => [
    { id: config_1.config.SD_MODEL, name: 'Stable Diffusion (Local)', type: 'image', provider: 'automatic1111' },
    ...(config_1.config.REPLICATE_API_TOKEN
        ? [{ id: 'stability-ai/sdxl', name: 'SDXL (Replicate)', type: 'image', provider: 'replicate' }]
        : []),
];
exports.imageModels = imageModels;
