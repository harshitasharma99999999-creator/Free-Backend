...
async function generateVideo(opts) {
    ...
    // ── Create prediction ──────────────────────────────────────────────────────
    const createRes = await fetch('https://api.replicate.com/v1/models/wan-ai/wan2.1-t2v-480p/predictions', {
        method: 'POST',
        headers: {
    ...
    for (let i = 0; i < 60; i++) {
        ...
        const pollRes = await fetch(`https://api.replicate.com/v1/predictions/${prediction.id}`, { headers: { 'Authorization': `Bearer ${config_1.config.REPLICATE_API_TOKEN}` } });
        const poll = await pollRes.json();
        logger_1.logger.debug({ id: prediction.id, status: poll.status }, 'Video generation status');
...
const videoModels = () => config_1.config.REPLICATE_API_TOKEN
